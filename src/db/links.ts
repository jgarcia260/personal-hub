  ...
  try {
    ...
    for (const title of linkedTitles) {
      try {
        ...
        // Link might already exist (UNIQUE constraint) or other error
        console.warn(`Failed to create link from ${noteId} to "${title}":`, error);
      }
    }
  } catch (error) {
    console.error(`Failed to update links for note ${noteId}:`, error);
  }
}
...
export async function getOutgoingLinks(noteId: string): Promise<{ id: string; title: string }[]> {
  ...
  try {
    ...
    console.error(`Failed to get outgoing links for note ${noteId}:`, error);
    return [];
  }
}
...
export async function getBacklinks(noteId: string): Promise<{ id: string; title: string }[]> {
  ...
  try {
    ...
    console.error(`Failed to get backlinks for note ${noteId}:`, error);
    return [];
  }
}
